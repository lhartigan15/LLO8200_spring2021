...
author: "Lacey Hartigan (adopted from Will Doyle)"
date: "1/26/21"
output:
...
at<-at%>%
  mutate(attrit=ifelse(Attrition=="Yes", 1, 0)) 
...
gg<-gg+geom_histogram(binwidth = 1, fill="darkblue")
gg
``` 
...
## Changing bandwidth--not recommended for this data, just showing you how. 
...
```{r}
#create a new dataset that just has the mean of our three groups
at_sum<-at%>% 
...
gg<-gg+geom_point(color="gold", size=5) #here's where you're telling R you want a dot plot
...
```{r}
## Changing Colors--note, this example just has TWO colors, so we need a plot that only needs two colors. Go up and re-run your bar plot for attitrion and gender code so that our "gg" only needs 2 colors. 
...
```{r}
gg<-ggplot(at_sum, aes(x=reorder(BusinessTravel, attr_avg), y=attr_avg, fill=Gender))
