...
The `crossv_kfold` command creates a list of datasets from our original dataset, each of which contains a testing and training dataset (without any overlap). The proportion of cases held out for testing is determined by the number of folds: 10 folds would indicate 1/10 of the data to be held out. 
...
*Not So quick exercise* From the pd dataset, choose new variables to add to your model to predict median home values. Compare the distribution of rmse from your model to the ones I obtained. 
```{r}
...
pd<-pd%>%
  select(-median_home_val) #drop out median home value b/c it's just a transformation of median home rank

#this is going to allow the computer to choose the predictors from the data
fitControl<-trainControl(method="boot", 
                         p=.2) 

#this is us choosing the predictors
...
##Stepwise Regression - computer adds predictors to improve fit (AIC - selects best measures for best model fit according to AIC [Akaike information criterion] value)
...
gg<-gg + geom_density(fill="orange", alpha=.2) #fit1 model - we specified two vars
gg<-gg + geom_density(aes(x=fit2), fill="blue", alpha=.2) #fit2 model - computer chose 4 vars
gg
...
There is a lot of great information about cross-validation in the caret package documentation here [https://topepo.github.io/caret/index.html]. There's additional useful info (and examples) here:
