...
gg1<-gg1 + geom_text(aes(label=round(prob_pizza, 2)), #add data labels to bars
                   position=position_dodge(width=.9), #matches positioning we originally specified above (see line 81); change the width# to adjust
                   vjust=-.25) #adjusts the height of the bars
gg1
```
...
za<-za%>%
  mutate(score_quintile=ntile(score, 5), #5 is quintile; what would quartile code be?; what would decile code be?
         karma_quintile=ntile(karma, 5))
#look at the data after you add these quintile vars
```
...
gg<-gg + scale_fill_gradient(low="light blue", high="dark blue") #fill with gradient coloring where lowest probabilities are black and highest gold
...
hypo_data<-data_grid(za, 
                     ...
                     karma=mean(karma, na.rm=TRUE), #hold constant
                     raop_posts=mean(raop_posts, na.rm=TRUE), #hold constant
